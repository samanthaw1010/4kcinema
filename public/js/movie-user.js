...
video_players.forEach((video_player) => {
        ...
        <div class="controls active">
            ...
            <div class="controls-list">
                ...
                <div class="controls-right">
                    ...
                    <span class="icon">
                        <i class="material-icons picture_in_picture">picture_in_picture</i>
                    </span>
    ...
    // Play video function + Pause video function
    function playVideo() {
        play_pause.innerHTML = 'pause';
        play_pause.title = 'Pause (space)';
    ...
    const insertIndex = Math.floor(qualities.length / 2); // Vị trí chèn giữa danh sách
    const insertAfterElement = quality_ul.children[insertIndex]; // Mục tồn tại sau đó chèn vào

    qualities.forEach((event) => {
        let quality_html = `<li data-quality="${event.getAttribute('size')}">${event.getAttribute('size')}</li>`;
    ...
    document.addEventListener('click', (event) => {
        ...
        const isClickInsideSettingsBtn = settingsBtn.contains(event.target);

        if (!isClickInsideSettings && !isClickInsideSettingsBtn) {
            settings.classList.remove('active');
            settingsBtn.classList.remove('active');
        }

        const isClickInsideCaptions = captions.contains(event.target);
        const isClickInsideCaptionsBtn = captionsBtn.contains(event.target);

        if (!isClickInsideCaptions && !isClickInsideCaptionsBtn) {
            captions.classList.remove('active');
            captionsBtn.classList.remove('active');
        }
    });
